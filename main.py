...
def resolve_steam_user(input_string):
    ...
    # If it's already a numeric ID64, use it directly
    if re.match(r"^\d{17}$", clean_input):
        steam_id_64 = clean_input
    else:
        # Resolve via Steam Vanity URL API call
        ...
        try:
            ...
            if res.get("response", {}).get("success") == 1:
                ...
                return {"error": f"Could not find a matching public Steam profile for name target: `{input_string}`"}
        except Exception:
...
# --- COMMAND 2: CLEAN DIRECT MENTION SEARCH ---
@bot.tree.command(name="steamid", description="Look up yours or a server member's Steam ID profiles seamlessly.")
@app_commands.describe(
    target_user="Tag a server member (e.g., @looksmaxxing) to automatically check their profile stats.",
    text_fallback="Alternatively, paste a friend's direct Steam URL link or exact custom text string here."
)
async def steamid(interaction: discord.Interaction, target_user: discord.Member = None, text_fallback: str = None):
    await interaction.response.defer(ephemeral=False)
    
    # 1. Identify what string data to search
    if target_user:
        # Use their server display name first, fallback to account username
        steam_target = target_user.display_name
    elif text_fallback:
        steam_target = text_fallback
    else:
        # Default to command sender if empty
        steam_target = interaction.user.display_name

    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, resolve_steam_user, steam_target)
    
    # 2. Multi-step fallback verification check
    ...
    if "error" in result:
        await interaction.followup.send(embed=discord.Embed(
            ...
            description=(
                f"{result['error']}\n\n"
                f"⚠️ **Why this happens:** If your friend's Discord profile name doesn't match their exact, custom "
                f"Steam profile URL name, the automated search engine can't guess it.\n\n"
                f"💡 **The Fix:** Simply run the command again and paste their direct Steam URL link straight "
                f"into the `text_fallback` box option prompt!"
            ), 
